...
public class A_VectorTest {
	...
	public static void main(String[] args) {
		...
		// add()와 addElement()의 차이점
		// => addElement()메서드는 데이터를 추가할 떄 동기화 작업을 수행한다.
		v1.addElement("ccc");
		...
		Vector<String> v2 = new Vector<>(); // String형만 저장 가능
		Vector<Integer> v3 = new Vector<>(); // int형만 저장 가능
		...
		// removeAll(Collection객체) => 벡터의 데이터 중에서 'Collection' 객체가 가지고 있는
		//                             데이터를 모두 삭제한다.
		v2.removeAll(v4);
		...
		// 벡터의 데이터들을 순서대로 모두 가져와서 사용하고 싶으면 반복문을 사용하면 된다.
		// (주로 for문 사용)
		...
		v.ensureCapacity(5); // 현재 용량이 최소용량보다 작다면...
							 // 신규용량 = 현재용량 * 2, 그래도 최소용량보다 작다면
		                     // 신규용량 = 최소용량으로 설정한다.
		System.out.println("== After ensureCapacity(5) ==");
		print(v);
		
		v.setSize(7); // 현재 용량이 설정 사이즈보다 작으면...
		              // 신규용량 = 현재용량 * 2, 그래도 설정사이즈보다 작다면
		              // 신규용량 = 설정사이즈로 설정된다.
