...
public class R_DialogTest extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		btnPopup.setOnAction(e -> {
			// Popup창에 나타날 컨트롤들을 구성 시작...
			HBox popRoot = new HBox();

			ImageView imgView = new ImageView();
			imgView.setImage(new Image(getClass().getResource("images/와나나.jpg").toString()));
			imgView.setFitWidth(300);
			imgView.setFitHeight(300);

			Label lbMsg = new Label("메세지가 왔습니다.");
			HBox.setMargin(lbMsg, new Insets(0, 5, 0, 5));

			popRoot.getChildren().addAll(imgView, lbMsg);
			// 구성끝

			// Popup객체 생성 후 뒤에서 구성한 컨트롤들 추가 후 보이기
			Popup popup = new Popup();
			popup.setAnchorX(200); //popup의 원하는 위치를 지정(Anchor 패널일때)
			popup.setAnchorY(200);
			popup.getContent().add(popRoot);
			popup.setAutoHide(true); //다른 행동을 하면 자동으로 숨겨짐
		...
		root.getChildren().addAll(btnFileOpen, btnFileSave, btnPopup, btnDirectory, btnCustom);
	...
	// 파일 열고싶으면 FILECHOOSER 경로는 열고싶으면 DIRCHOOSER
