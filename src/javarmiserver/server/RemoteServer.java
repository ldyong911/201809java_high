...
import javarmiserver.inter.RemoteInterface;
...
public class RemoteServer extends UnicastRemoteObject implements RemoteInterface{
	...
	// RMI용 인터페이스를 구현한 객체의 생성자 RemoteException을 throws하도록 작성한다.
	...
	public static void main(String[] args) {
		try {
			...
			// 형식) Registry객체변수.rebind("객체의Alias", 등록할 객체변수);
	...
	public void doPrintVo(TestVO vo) throws RemoteException {
		System.out.println("클라이언트에서 보낸 TestVO객체의 값 출력");
	...
	public void setFiles(FileInfoVO[] fInfo) throws RemoteException {
		...
		for(int i=0; i<fInfo.length; i++) {
			try {
				fout = new FileOutputStream(dir + fInfo[i].getFileName());
				
				// 클라이언트에서 전달한 파일데이터(byte[])를 서버측에 저장한다.
				fout.write(fInfo[i].getFileData());
				fout.close();
			} catch (IOException e) {
				e.printStackTrace();
			}
